    ...
    public class Script
    {
        ...
        public void Execute(ScriptContext context /*, System.Windows.Window window, ScriptEnvironment environment*/)
        {
            ...
            //{
                ...
                Structure BODY = null;
                foreach (Structure str in PlanStructures.Where(s => s.Id.ToLower().Equals("body")))
                {
                    switch (!str.IsEmpty)
                    {
                        case true:
                ...
                foreach (Structure str in PlanStructures.Where(s => s.Id.ToLower().Equals("couchsurface")))
                {
                    switch (!str.IsEmpty)
                    {
                        case true:
                ...
                MessageBox.Show(ptInfo + Environment.NewLine + Environment.NewLine 
                    + "Table surface to Farthest skin surface distance (A) = " + Environment.NewLine + "\t" + A.ToString("0.00") + " cm" 
                    + Environment.NewLine + Environment.NewLine + "Table surface to isocenter or prostate / prostatic bed center distance (B) = " + Environment.NewLine + "\t" + B_Y_Min.ToString("0.00") + " cm" 
                    + Environment.NewLine + Environment.NewLine + "Farthest Skin surface to prostate / prostatic bed or isocenter distance (A – B) = " + Environment.NewLine + "\t" + (A - B_Y_Min).ToString("0.00") + " cm" 
                    + Environment.NewLine + Environment.NewLine + "Table surface to closest transponder distance (C) = " + Environment.NewLine + "\t" + (C_Y_Min).ToString("0.00") + " cm" 
                    + Environment.NewLine + Environment.NewLine + "Skin Surface to closest transponder distance (A – C) = " + Environment.NewLine + "\t" + (A - C_Y_Min).ToString("0.00") + " cm"
